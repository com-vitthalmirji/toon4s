...
object Encoder {
  ...
  implicit val doubleEncoder: Encoder[Double]     = (d: Double) => if (d.isInfinity || d.isNaN) JNull else JNumber(BigDecimal(d))
  implicit val floatEncoder: Encoder[Float]       = (f: Float) => if (f.isInfinite || f.isNaN) JNull else JNumber(BigDecimal.decimal(f))
  ...
  implicit def mapEncoder[A](implicit ev: Encoder[A]): Encoder[Map[String, A]] =
    (m: Map[String, A]) => JObj(VectorMap.from(m.iterator.map { case (k, v) => k -> ev.apply(v) }))
  ...
  implicit def productEncoder[A <: Product]: Encoder[A] = (p: A) => {
    ...
    val kvs    = names.zip(values).map { case (k, v) => k -> anyToJson(v) }.toList
  ...
  private def anyToJson(v: Any): JsonValue = v match {
    ...
    case None                    => JNull
    case m: Map[_, _]            =>
      val it = m.asInstanceOf[Map[String, Any]].iterator.map { case (k, v) => k -> anyToJson(v) }
