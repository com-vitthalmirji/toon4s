 ...
 * 
 * Provides an interactive interface for converting between JSON and TOON formats,
 * demonstrating token savings and format features.
 */
object PlaygroundApp {
  ...
  private def updateStats(jsonText: String, toonText: String): Unit = {
    ...
    getElement("statsJsonSize").foreach { elem =>
      elem.textContent = s"$jsonChars chars"
    }
    getElement("statsToonSize").foreach { elem =>
      elem.textContent = s"$toonChars chars"
    }
    ...
    getElement("statsSection").foreach { section =>
